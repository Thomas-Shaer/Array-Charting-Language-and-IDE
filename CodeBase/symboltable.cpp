...
std::shared_ptr<SymbolTable> SymbolTable::GLOBAL_SYMBOL_TABLE = std::make_shared<SymbolTable>(
	...
	std::map<std::string, MethodBucket*>(
	{
		...
		// registers the method on the global symbol table
...
SymbolTable::SymbolTable(std::map<std::string, MethodBucket*> _methodTable,
	...
	// USED to extract build in method information 
	// for the final report

	/*for (auto method : _methodTable) {
		SingleMethodBucket* singlemethodbucket = dynamic_cast<SingleMethodBucket*>(method.second);
		if (singlemethodbucket) {
			std::cout << method.first << " @ " << METHOD_CAT_TO_STRING.at(method.second->methodCat) << " @ " << method.second->toString() << " | " << singlemethodbucket->methodsymbol->description << std::endl;
		}
		else {
			OverloadedMethodBucket* overloaded = dynamic_cast<OverloadedMethodBucket*>(method.second);

			std::string overloads = "";

			for (MethodSymbol* meth : overloaded->overloads) {
				overloads += meth->getSignature() + " | " + meth->description + " | ";
			}

			std::cout << method.first << " @ " << METHOD_CAT_TO_STRING.at(method.second->methodCat) << " @ " << overloads << std::endl;
